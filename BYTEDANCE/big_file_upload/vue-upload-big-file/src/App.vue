...
export default {
  ...
  filters: {
    transformByte(val) {
      return Number((val/1024).toFixed(0))//去掉小数点后的数字
    }
  },
  computed: {
    uploadPercentage() {
  ...
  methods:{
    ...
    resetData(){
      this.requestList.forEach(xhr => xhr? xhr.abort():null)//forEach 每个都执行
      this.requestList = [];
      if (this.container.worker){ // hash 计算过程中
      this.container.worker.onmessage = null;//停下来了
      }
    },
    request({
            url,
    ...
    async uploadChunks (uploadedList = []) {
      ...
      const requestList = this.data
        .map(({ chunk, hash, index}) => {
          ...
          formData.append("fileHash", this.container.hash)// 大文件hash
